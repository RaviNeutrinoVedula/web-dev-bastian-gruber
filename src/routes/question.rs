...
pub async fn add_question(
    ...
    match store.add_question(question, account_id)
        .await {
...
pub async fn update_question(
    ...
    let account_id = session.account_id;
    if store.is_question_owner(id, &account_id)
	.await? {
		...
		match store.update_question(question, id, account_id)
		    .await {
			Ok(res) => Ok(warp::reply::json(&res)),
			Err(e) => Err(warp::reject::custom(e)),
		    }
...
pub async fn delete_question(
    ...
    if store.is_question_owner(id, &account_id)
	.await? {
	    match store.delete_question(id, account_id)
		.await {
